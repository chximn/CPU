...
MemoryOperand::MemoryOperand(register_code b,
                             register_code i,
                             uint8_t s,
                             uint64_t d,
                             uint8_t si,
...
MemoryOperand::MemoryOperand(register_code i,
                             uint8_t s,
                             uint64_t d,
                             uint8_t si,
                             register_code seg):
...
std::string MemoryOperand::to_string() const {
    ...
    if (use_segment)
        str += Register::to_string(segment) + ":";
    str += "[";
    if (use_base)
        str += Register::to_string(base);
    if (use_index) {
        if (use_base)
            str += " + ";
    ...
    if (displacement != 0) {
        if (use_index || use_base)
            str += " + ";
        str += helpers::to_hex(displacement);
    }

    else if (!use_index && !use_base)
        str += "0";
...
std::string SymbolOperand::get_name() const {
    return name;
}
int SymbolOperand::get_line() const {
    return line;
}
uint8_t SymbolOperand::get_size() const {
    return 0;
}
std::string SymbolOperand::to_string() const {
    return name;
}

LabelOperand::LabelOperand(std::string const & s, int l): SymbolOperand(s, l) {}
ObjectOperand::ObjectOperand(std::string const & s, int l):
    SymbolOperand(s, l) {}
