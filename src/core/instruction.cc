...
std::string Instruction::to_string(instruction_code code) {
    std::string names[] = {
        ...
        "fld", "fldz", "fld1", "fst", "fstp", "fadd", "faddp",  //
                                                                //
        "movdqu", "movdqa", "paddb", "paddw", "paddd", "paddd", "addps",
        "addpd", "pand", "pshufd"  //
    };
...
Instruction::Instruction(instruction_code c,
...
Instruction::Instruction(Instruction const & i):
    code(i.get_code()), size(i.get_size()) {
