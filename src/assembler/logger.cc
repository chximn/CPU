...
void Logger::error(std::string const & s, int line) {
    ...
    for (int i = std::max(1, line - LOGGER_PADDING); i < line; i++)
        std::cerr << "\e[90m" << format_line_number(i) << " |\t"
                  << lines.at(i - 1) << "\e[0m\n";

    std::cerr << "\e[91m" << format_line_number(line) << "\e[0m |\t"
              << lines.at(line - 1) << "\e[0m\n";

    int size = lines.size();
    for (int i = line + 1, l = std::min(size, line + LOGGER_PADDING - 1); i < l;
         i++)
        std::cerr << "\e[90m" << format_line_number(i) << " |\t"
                  << lines.at(i - 1) << "\e[0m\n";
